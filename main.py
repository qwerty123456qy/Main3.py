                        ...
                        if '0300' in dataS.hex()[0:4] :
                            ...
                            if '0500' in dataS.hex()[:4] and b'\x05\x15\x00\x00\x00\x10Z\xca\xf5&T;\x0cA\x01\x16\xe0\x05\xb2\xea\xe4\x0b' in dataC:
                                ...
                                if '1200' in dataS.hex()[0:4] and'2f696e76'  in dataS.hex()[0:900] : 
                                    ...
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FF00][b][c]  invite : ON")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c] invite : ON"))))
                                    inviteD =True
                               #     time.sleep(3.5)
                    ...
                    #            if '1200' in dataS.hex()[0:4] and '666f7879' in dataS.hex()[0:900] :
                         ...
                         #           client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c]Instagram : [FFC800][b][c]@the_foxy999"))))
                                ...
                                if '1200' in dataS.hex()[0:4] and '2f2d646573' in dataS.hex()[0:900] :
                                    inviteD =False
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[FF0000][b][c]invite : OFF")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[FF0000][b][c]invite : OFF"))))                       
                       ...
                       #         if '1200' in dataS.hex()[0:4] and '3f2d6c766c' in dataS.hex()[0:900] :
                           ...
                           #         client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[FF0000][b][c]Stopped !"))))
                                ...
                                #spy_last_sqoud
                                if '1200' in dataS.hex()[0:4] and '2f31' in dataS.hex()[0:900] :
...
#           /5
                                if '1200' in dataS.hex()[0:4] and '2f35' in dataS.hex()[0:900]:
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FF00][b][c] 5 in group")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c]5 in group"))))
                          ...
                          #      if '1200' in dataS.hex()[0:4] and '2f32' in dataS.hex()[0:900]:
                             #       client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FFFF][b][c] Send Mode 2 ok!")))
                                #    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FFFF][b][c]Send Mode 2 ok!"))))
                                    #client.send(dataS)
                                  #  invite.send(bytes.fromhex(b'\x05\x15\x00\x00\x00 \xf3\x7f\x06i,\x9d\xbe$Z\xf3|\xb3\xdfO\xc5\xf4\x8bT\x8b\xf7Y\x1b\xe3\x8cY \x93:\x88\xa6\xfd\\'))




                                if '1200' in dataS.hex()[0:4] and '2f73' in dataS.hex()[0:900] and spaming:
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FF00][b][c]SPAM : ON")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c]SPAM : ON"))))
                                ...
                                if '1200' in dataS.hex()[0:4] and '2f66' in dataS.hex()[0:900]:
                                    recordmode=False
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[FF0000][b][c]SPAM : OFF")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[FF0000][b][c]SPAM :  OFF"))))
...
def server():
    try:
        ...
        r = requests.get(url="https://raw.githubusercontent.com/Yyhhfi/botv.1-/main/control%20bot.1").text
        
        
        if "3757275727561947" in r or "374727573756" in str(r):
